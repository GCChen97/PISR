...
class BaseImplicitGeometry(BaseModel):
    ...
    def isosurface(self):
        if self.config.isosurface is None:
            raise NotImplementedError
        mc_range = getattr(self, 'mc_range', 1.0) # set by system.pol_neus.test_epoch_end
        if isinstance(mc_range, float):
            radius_ = self.radius * mc_range
            mesh_coarse = self.isosurface_((-radius_, -radius_, -radius_), (radius_, radius_, radius_))
        elif isinstance(mc_range, list):
            mesh_coarse = self.isosurface_(mc_range[0], mc_range[1])
        else:
            print("[ERROR] Unknown mc_range in volume-sdf.isosurface")
